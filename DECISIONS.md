...
## ADR-041 — Timings are published, and deliberately outside the reproducibility guarantee

**Date:** 2026-09-04
**Status:** Accepted. Corrects the first attempt, which CI rejected within a minute.

**Context:** Track 04's bar is *"throughput plus measured accuracy plus an honest exception
list"*, and the guide is explicit: report how many matched, **how fast**, and which could
not be resolved. `RunMetrics.throughput` has existed since day 8 carrying a docstring that
reads *"the track bar asks for it explicitly"* — and no table ever rendered it. Measured
every run since, discarded every run since. A third of the stated bar sat in memory and
never reached the page.

**The obvious fix was wrong, and CI caught it.** Adding a throughput column to
`results/summary.md` and `results/metrics.md` put a timing inside the byte-identical drift
check that ADR-036 established. The same commit measured **1,340 credits/s** on the
development laptop and **952/s** on the GitHub runner. The build failed on the first push.

That failure is the useful part. A timing is an observation about a machine under a
particular load; every other published figure is a property of the code and reproduces
exactly. Holding the two to one standard leaves two options, and both are bad: a
permanently red build, or a guarantee loosened for everything so one number can live inside
it. The guarantee is what makes the other figures worth trusting.

**Decision:** throughput is published in `results/throughput.md`, generated by the same
command, and **excluded from the drift check by name** in CI with a comment saying why. The
file states its own limits in its header: not byte-reproducible, and it names the machine
it was measured on.

**Alternatives considered:**
- **Round it into a bucket** so both machines agree. Lost: the bucket that survives a loaded
  CI runner is so coarse it stops being a measurement.
- **Drop throughput.** Lost: the track bar names it.
- **Drop the drift check.** Refused. It has already caught a partial table being published
  over a complete one.

**Consequences:** the honest comparison it enables is worth having. On `adversarial` the
full cascade runs at roughly 1,300 credits/s against the LLM-only baseline's ~1,100 — but
**both arms are replayed from the committed cache, so neither number includes API latency**,
and the file says so. Read as matcher work the comparison is fair; read as wall-clock it
flatters both, and it flatters the baseline most, because that arm makes the most calls. At
the free tier's pacing its 165 live calls take minutes, not the second shown.

Publishing the flattering reading without that caveat would have been ADR-033's mistake a
